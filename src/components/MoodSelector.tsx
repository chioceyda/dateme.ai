...
export const MoodSelector = ({ currentMood, onMoodChange }: MoodSelectorProps) => {
  return (
    <div className="flex items-center space-x-2">
      ...
      <Select value={currentMood} onValueChange={(value) => onMoodChange(value as ChatMood)}>
        <SelectTrigger className="w-32 h-8 text-sm bg-white border-pink-200">
          <SelectValue />
        </SelectTrigger>
        <SelectContent className="bg-white border-pink-200">
          {Object.entries(moodEmojis).map(([mood, emoji]) => (
            <SelectItem key={mood} value={mood} className="cursor-pointer hover:bg-pink-50">
